...
def conv2d(x, num_filters, name, filter_size=(3, 3), stride=(1, 1), pad="SAME", dtype=tf.float32, collections=None,
    ...
    with tf.variable_scope(name):
        ...
        w = get_variable("W", filter_shape, dtype, tf.random_uniform_initializer(-w_bound, w_bound),
                            collections=collections)
        b = get_variable("b", [1, 1, 1, num_filters], initializer=tf.zeros_initializer,
                            collections=collections)
...
def dense(x, size, name, weight_init=None, bias=True, trainable=True):
    initializer = lambda: weight_init(size)
    w = get_variable(name + "/w", shape=size, trainable=trainable, initializer=initializer)
    ret = tf.matmul(x, w)
    if bias:
        b = get_variable(name + "/b", [size[1]], trainable=trainable, initializer=weight_init)
        return ret + b
    else:
        return ret

def wndense(x, size, name, init_scale=1.0):
    v = get_variable(name + "/V", [int(x.get_shape()[1]), size],
                        initializer=tf.random_normal_initializer(0, 0.05))
    g = get_variable(name + "/g", [size], initializer=tf.constant_initializer(init_scale))
    b = get_variable(name + "/b", [size], initializer=tf.constant_initializer(0.0))
...
def batchnorm(x, name, phase, updates, gamma=0.96):
    k = x.get_shape()[1]
    runningmean = get_variable(name+"/mean", shape=[1, k], initializer=tf.constant_initializer(0.0), trainable=False)
    runningvar = get_variable(name+"/var", shape=[1, k], initializer=tf.constant_initializer(1e-4), trainable=False)
    ...
    out = y * get_variable(name+"/scaling", shape=[1, k], initializer=tf.constant_initializer(1.0), trainable=True)\
            + get_variable(name+"/translation", shape=[1,k], initializer=tf.constant_initializer(0.0), trainable=True)
    return out
